...
export default async function handler(
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      console.warn(`⚠️ Rate limit exceeded for request ${requestId}`);
      res.status(429).json(
    ...
    console.error(`❌ Unexpected error in ecosystem stats API ${requestId}:`, error);
...
async function handleGetRequest(
  ...
  try {
    ...
    console.error(`❌ Error in GET request ${requestId}:`, error);
    res.status(500).json(
      createApiResponse(
        false,
        null,
...
async function handlePostRequest(
  ...
  try {
    ...
    console.error(`❌ Error in POST request ${requestId}:`, error);
    res.status(500).json(
      createApiResponse(
        false,
        null,
...
async function handleDeleteRequest(
  ...
  try {
    ...
    console.error(`❌ Error in DELETE request ${requestId}:`, error);
    res.status(500).json(
      createApiResponse(
        false,
        null,
...
async function getGrowthData(timeRange: TimeRange, refreshCache = false) {
  console.log(`📈 Fetching growth data for ${timeRange}...`);
...
async function getDevelopersData(limit: number, refreshCache = false) {
  console.log(`👥 Fetching developers data (limit: ${limit})...`);
