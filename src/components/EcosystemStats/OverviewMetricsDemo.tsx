...
const OverviewMetricsDemo: React.FC<DemoProps> = ({ className = '' }) => {
  ...
  const updateMockData = () => {
    ...
    const newData: EcosystemOverview = {
      ...
      healthScore: Math.max(0, Math.min(100, (mockData.healthScore || 50) + Math.floor(Math.random() * 10) - 5)),
  ...
  const MockOverviewMetricsWrapper = () => {
    ...
    const [error, setError] = useState<string | null>(demoState === 'error' ? 'Demo error state' : null);
    const [data, setData] = useState<EcosystemOverview | null>(demoState === 'error' ? null : mockData);
      ...
      <div className="card p-6">
        ...
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-3">
            Expected response format follows the <code className="text-xs bg-gray-100 dark:bg-gray-700 px-1 rounded">EcosystemStatsResponse&lt;EcosystemOverview&gt;</code> type.
          </p>
        </div>
      </div>
    </div>
