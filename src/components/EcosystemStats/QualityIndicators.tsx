...
import { QualityIndicators as IQualityIndicators, EcosystemStatsResponse } from '../../types/ecosystem-stats';
...
const TrendIndicator: React.FC<{
  ...
  const bgColorClass = trend === 'up' ? 'bg-success-100 dark:bg-success-900/30' : 'bg-error-100 dark:bg-error-900/30';
...
const TrustBadge: React.FC<{
  ...
  return (
    ...
    >
      <div className={`p-2 rounded-lg ${signal.color} transition-all duration-200 group-hover:scale-110 cursor-help`}>
        <Icon className={sizeClasses[size]} />
      </div>
...
const QualityIssueCard: React.FC<{
  ...
  return (
    <div className={`p-3 rounded-lg border ${colors.bg} ${colors.border} ${compact ? 'text-xs' : 'text-sm'}`}>
      <div className="flex items-start justify-between">
        <div className={`font-medium ${colors.text}`}>
          {issue.issue}
        </div>
        <div className={`text-xs ${colors.text} opacity-75`}>
          {issue.frequency} cases
        </div>
      </div>
...
const QualityIndicators: React.FC<QualityIndicatorsProps> = ({
  ...
  const { data, loading, error, lastUpdated, refetch } = useQualityIndicators(autoRefresh, refreshInterval);
  ...
  const getQualityMetrics = (): QualityMetric[] => {
    ...
    return [
      ...
      },
      {
        ...
        trend: 'stable',
        icon: Lock,
        color: data.security?.securityScore && data.security.securityScore >= 80 ? 'success' : 'warning',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          ...
          <div className="text-sm text-gray-600 dark:text-gray-400">
            <p className="mb-2">
              <strong>{data?.verification?.verifiedPlugins || 0}</strong> of <strong>{(data?.verification?.verifiedPlugins || 0) + 100}</strong> plugins are verified
            </p>
            <p>
              Verification ensures plugins meet security, quality, and maintenance standards.
            </p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-primary-50 dark:bg-primary-900/20 rounded-lg">
                <div className="text-2xl font-bold text-primary-600 dark:text-primary-400">
                  {((data.qualityMetrics.highQualityPlugins / (data.qualityMetrics.highQualityPlugins + 50)) * 100).toFixed(1)}%
                </div>
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  Quality Rate
                </div>
            ...
            <div className="card p-6">
              ...
              <div className="mt-4">
                <ProgressBar
                  ...
                  color={data.security.securityScore >= 80 ? 'success' : data.security.securityScore >= 60 ? 'warning' : 'error'}
                  size="md"
