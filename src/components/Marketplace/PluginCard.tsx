...
const PluginCard: React.FC<PluginCardProps> = ({ plugin, className = '' }) => {
  ...
  const { handleClick, handleDownload, handleShare } = useClickTracking(plugin.id, 'plugin', plugin.name);
      ...
      <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 mb-4">
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-1 group">
            <Star className="w-4 h-4 group-hover:fill-current group-hover:text-yellow-500 transition-colors" aria-hidden="true" />
            <span className="font-medium">{formatNumber(plugin.stars)}</span>
          </div>
          <div className="flex items-center space-x-1 group">
            <Download className="w-4 h-4 group-hover:text-green-500 transition-colors" aria-hidden="true" />
